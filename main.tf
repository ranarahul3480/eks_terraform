...
resource "aws_subnet" "rahul_subnet" {
  count                   = 2
  vpc_id                  = aws_vpc.rahul_vpc.id
  cidr_block              = cidrsubnet(aws_vpc.rahul_vpc.cidr_block, 8, count.index)
  availability_zone       = element(["ap-south-1a", "ap-south-1b"], count.index)
  map_public_ip_on_launch = true

  tags = {
    Name = "rahul-subnet-${count.index}"
  }
}
...
resource "aws_route_table" "rahul_route_table" {
  vpc_id = aws_vpc.rahul_vpc.id
  ...
  tags = {
    Name = "rahul-route-table"
  }
}

resource "aws_route_table_association" "a" {
  count          = 2
  subnet_id      = aws_subnet.rahul_subnet[count.index].id
  route_table_id = aws_route_table.rahul_route_table.id
}

resource "aws_security_group" "rahul_cluster_sg" {
  vpc_id = aws_vpc.rahul_vpc.id
  ...
  tags = {
    Name = "rahul-cluster-sg"
  }
}

resource "aws_security_group" "rahul_node_sg" {
  vpc_id = aws_vpc.rahul_vpc.id
...
resource "aws_eks_cluster" "rahul" {
  ...
  vpc_config {
    subnet_ids         = aws_subnet.rahul_subnet[*].id
    security_group_ids = [aws_security_group.rahul_cluster_sg.id]
  }
}
...
resource "aws_eks_node_group" "rahul" {
  ...
  remote_access {
    ...
    source_security_group_ids = [aws_security_group.rahul_node_sg.id]
  }
}
